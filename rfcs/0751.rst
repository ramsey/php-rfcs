...
Class constants provide a way to define static values on a class. Unlike
class properties, class constants cannot be typed, and have no
inheritance or ways to ensure extending or implementing classes have
specific constants.

This RFC is proposing typed class constants along with inheritance to
ensure concrete classes have contracted constants.
...
Class constants will also support all union types, as long as each union
type is one of the supported types. For example, these would all be
valid:
...
constants are immutable. The type check will always be performed when a
constant is typed. This is consistent with the handling of typed
...
Class constants will be allowed to be declared with no value in abstract
classes and interfaces. Concrete classes extending the abstract class or
implementing an interface with a declared constant with no value must
set a value.
...
:Original Authors: Benas Seliuginas benas.molis.iml@gmail.com, Mark Niebergall mbniebergall@gmail.com
