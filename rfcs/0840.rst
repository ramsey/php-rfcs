...
(``Collections\Deque``) than the ``SplDoublyLinkedList`` would save
users from needing to write code with these pitfalls):
...
This proposes adding the class ``final class Collections\Deque`` to PHP.
From `the Wikipedia article for the Double-Ended
...
``Collections\Deque`` is an ``IteratorAggregate`` where ``getIterator``
returns an ``InternalIterator``. Different parts of the code can
independently iterate through instances of the same ``Deque`` (e.g.
nested for loops).
...
raised), they would benefit from the speedup and reduced memory that
``Collections\Deque`` has over ``SplDoublyLinkedList``/``SplQueue``.
...
introducing a data structure (``Deque``) that's even faster and uses
less memory than an ``array`` for use as a double-ended queue, even more
applications would benefit from it.
...
``array_push()``/``array_pop()``/``array_shift()``/``array_unshift()``/``array_key_first()``/``array_key_last()``,
...
takes 15.030 seconds, and ``Collections\Deque`` takes 0.126 seconds for
the 2000 iterations (``Deque`` is 120 times faster for ``n=1000``).**
...
(`unshift`) to the start of an array. ``array_unshift`` is a linear time
...
start(first elements) of an array. (Shifting or unshifting (or
unsetting) the start is inefficient for arrays)
...
8.2's feature freeze (probably in July 2022) (e.g.
``insert(int $offset, mixed ...$values)``, ``remove(int $offset)``)
...
**Also, this isn't a request specifically about ``Collections\Deque`` or
collections in general, but rather about the general design of classes
in php**
...
of ``$deque->unshift()``/``push()`` with the return type info being an
object)
...
general, not for push/unshift), then it's less of a breaking change to
