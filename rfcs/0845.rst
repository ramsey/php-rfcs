...
This RFC proposes to allow use of ``->`` to fetch properties in constant
expressions. The primary motivation for this change is to allow fetching
the ``name`` and ``value`` properties of enums in places where enum
objects aren't allowed, like array keys (see
