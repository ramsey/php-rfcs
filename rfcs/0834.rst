...
-  ＿construct(string|int|null $seed = null)
-  generate(): string
...
-  The number of calls to the Engine::generate() method remains the
   same.
-  The return value remains the same for a given　result retrieved
   from　Engine::generate().

Any changes to the Randomizer that violate these guarantees require
a　separate RFC.
