...
-  Make the boolean type and the type juggling system safer and more
   consistent
-  Set up only 7 scalar values as unambiguous boolean values is easy to
...
.. code:: php
   ...
   $obj->boolProperty = strlen($string) > 0; // this would make the intent to check for an empty string clear

This RFC does establish a more narrow view for implicit boolean
conversions only for typed booleans, because in that context you are
reducing an int, float or string to a boolean, possibly losing
information, and not evaluating an expression where there is follow-up
code to do something more as is the case with ``if``, ``?:`` or ``&&`` /
``||``.
